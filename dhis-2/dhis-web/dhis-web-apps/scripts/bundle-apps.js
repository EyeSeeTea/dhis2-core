...
const { copy_local_app, is_local_app } = require("./local.js");
...
/**
 * Process a single app - either clone from git or copy from local
 */
async function process_app(app, target, default_branch) {
  if (is_local_app(app)) {
    return copy_local_app(app, target);
  } else {
    return clone_app(app, target, default_branch);
  }
}
...
async function main(opts = {}) {
  ...
  const html_index_path = path.join(
    build_dir,
    artifact,
    artifact,
    "index.html"
  );
  const xml_template_path = path.join(
    root,
    "src",
    "main",
    "resources",
  ...
  try {
    ...
    console.log(
      `${path.basename(bundle_path)} exists; re-use cached apps for bundle`
    );
    process.exit(0);
  } catch (err) {
    console.log(
      `${path.basename(bundle_path)} doesn't exist; run full bundle operation`
    );
  }
