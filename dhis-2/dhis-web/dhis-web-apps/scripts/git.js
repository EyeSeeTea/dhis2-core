...
async function checkout(name, path, treeish) {
  ...
  while (retries <= 100) {
    try {
      ...
      if (retries >= 100) {
        console.error(
          `[checkout] [${name}] could not find '${treeish}' in the last 1000 commits .. are you sure it's there?`
        );
        process.exit(1);
      }
...
async function clone(name, url, clone_path, treeish) {
  ...
  let retries = 0;
  try {
    // Naively try as branch first, this will fail if treeish is a commit-sha
    await exec(`git clone --depth 1 --branch ${treeish} ${url} ${clone_path}`);
    console.log(`[clone] [${name}] shallow clone successful`);
  } catch (err) {
    console.log(
      `[clone] [${name}] shallow clone failed. Starting clone with --no-single-branch. This may take some time`
    );
    while (true) {
      try {
        // Clone other branches as well, in case of commit sha
        await exec(
          `git clone --depth 1 --no-single-branch ${url} ${clone_path}`
        );
        ...
        if (retries >= 10) {
          console.error(
            `[clone] [${name}] there was a problem with the clone operation`,
            err
          );
          process.exit(1);
        }
...
async function get_sha(repo_path) {
  ...
  try {
    ...
    console.error(
      `[get_sha] could not fetch refspec for ${repo_path}, using fallback`,
      err
    );
    refspec = "n/a";
  }
...
async function get_commit_date(repo_path, sha) {
  ...
  try {
    ...
    console.error(
      `[get_commit_date] could not fetch commit date for ${repo_path}, using fallback`,
      err
    );
    refspec = "n/a";
  }
...
module.exports = { clone_app, get_sha, get_commit_date };
